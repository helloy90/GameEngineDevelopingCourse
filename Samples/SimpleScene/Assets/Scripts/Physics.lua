...
local interactQuery = ecs.query("Position, Velocity, Collider")

local function InteractSystem(it)
	for projPos, projVelocity, projCollider, projEnt in ecs.each(it) do
		for objPos, objVelocity, objCollider, objEnt in ecs.each(interactQuery) do
			if projEnt ~= objEnt then
				if near(projPos, objPos, 1.0) then
					objVelocity.x = -objVelocity.x * 0.8
					objVelocity.y = -objVelocity.y * 0.8
					objVelocity.z = -objVelocity.z * 0.8

					objPos.x = objPos.x + objVelocity.x * 0.1
					objPos.y = objPos.y + objVelocity.y * 0.1
					objPos.z = objPos.z + objVelocity.z * 0.1

					projVelocity.x = -projVelocity.x * 0.3
					projVelocity.y = -projVelocity.y * 0.3
					projVelocity.z = -projVelocity.z * 0.3

					projPos.x = projPos.x + projVelocity.x * 0.1
					projPos.y = projPos.y + projVelocity.y * 0.1
					projPos.z = projPos.z + projVelocity.z * 0.1
				end
			end
		end
	end
...
ecs.system(InteractSystem, "InteractSystem", ecs.OnUpdate, "Position, Velocity, Collider")
